...
public class UserRegistrationService {
    ...
    public CommonResult register(String email, String password, String verificationCode) {
        ...
        if (code == null) {
            return CommonResult.error("验证码已过期").setAction(-1);
        }
        if (!code.equals(verificationCode)) {
            return CommonResult.error("验证码错误").setAction(-2);
        }
        //先查询是否已经被注册
        User user = userDao.queryByEmail(email);
        //这里是为了防止有人同时申请注册 所以这里也需要判断
        if (user != null) {
            return CommonResult.error("该邮箱已被注册").setAction(-3);
        }
    ...
    public CommonResult applyToRegisterAccount(String email) {
        ...
        //邮箱已注册
        if (user != null) {
            return CommonResult.error("该邮箱已被注册");
        }
        //生成随机验证码
        ...
        return CommonResult.ok("验证码已发送至邮箱")
                .set("email", email)
